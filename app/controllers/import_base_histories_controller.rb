class ImportBaseHistoriesController < CustomUserApplicationController
  ...
  def index
    ...
    @sort_url = import_base_histories_path
    ...
    if @sort_field_name
      render turbo_stream: [
        turbo_stream.replace("entry-turbo-import_histories_search_results", partial: 'import_base_histories/search_results'),
      ]
    else
      render :index
    end
